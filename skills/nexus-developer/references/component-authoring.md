...
Todo componente novo **precisa** de uma entrada em `packages/cli/src/core/registry/data/<category>.ts`. Sem isso:
...
- [ ] Entrada adicionada em `packages/cli/src/core/registry/data/<category>.ts` — `name`, `basePath`, `files`, `registryDependencies`, `dependencies`.
