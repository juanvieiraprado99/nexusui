...
@Component({
  ...
  template: `
    ...
    <div class="relative flex flex-1 items-center" data-slot="control-wrapper">
      ...
      @if (hasTrailingIcons()) {
        <div [class]="trailingIconsClasses()">
          ...
          @if (nCopyable()) {
            <button
              ...
              (click)="handleCopy()"
              [attr.aria-label]="showCopySuccess() ? 'Copied' : 'Copy'"
            >
              @if (showCopySuccess()) {
...
export class InputGroupComponent {
  ...
  constructor() {
    effect(() => {
      ...
      // Propagate the group's disabled state to the projected control so the
      // native `:disabled` (and the `has-disabled:` styles) actually apply.
      this.renderer.setProperty(el, 'disabled', this.nDisabled() || this.nLoading());
      ...
      this.renderer.setAttribute(el, 'data-ig-prev', classes);
    });
