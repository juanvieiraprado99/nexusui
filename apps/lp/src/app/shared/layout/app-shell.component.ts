          ...
          <div class="ml-auto flex items-center gap-2">
            <div class="hidden lg:flex relative">
              <input
                ...
                class="h-8 w-64 rounded-md border border-border bg-muted/40 px-3 text-xs text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-ring"
                aria-label="Search documentation"
              />
              <kbd class="absolute right-2 top-1/2 -translate-y-1/2 inline-flex h-5 items-center rounded border border-border bg-background px-1.5 text-[10px] font-mono text-muted-foreground">⌘K</kbd>
            </div>
            <button
              type="button"
              n-button
            ...
            <button n-button nSize="sm" type="button">+ New</button>
          </div>
        </div>
      </header>
...
export class AppShellComponent {
  ...
  protected readonly navLinks = [
    ...
    { path: '/directory', label: 'Directory', exact: false },
  ];
}
